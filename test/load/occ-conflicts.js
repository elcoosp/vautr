...
import { readTarget, sessionSetup } from './lib.js';
...
export default function (data) {
  const itemUuid = data.targetUuid || readTarget(data, null).uuid; // resolve once via pull
  ...
  check(res, {
    ...
    'resolved version == target+1 (OCC correct)': () => {
      if (!r0) return false;
      const resolved = r0.status === 'success' ? r0.version : r0.current_server_state?.version;
