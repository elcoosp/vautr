/**
 * Development mock of the popup's `vautr-wasm` `WebClient` (`--target web`).
 *
 * Mirrors the real wasm-pack output from `core/vautr-wasm` for the popup-scoped
 * Web Worker. Performs NO real cryptography; demonstrates the opaque-handle /
 * perform_action boundary (client.md §4). Swap the Vite alias to the real
 * built module to exercise actual crypto.
 */
