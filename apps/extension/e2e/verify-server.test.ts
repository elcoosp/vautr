...
const pr=await fetch(`${BASE}/projects`,{method:'POST',headers:hdr,body:JSON.stringify({name:'Proj A',type:'personal'})});
...
const sr=await fetch(`${BASE}/projects/${proj.uuid}/secrets`,{method:'POST',headers:hdr,body:JSON.stringify({key:'DB_PASS',value_ciphertext:'enc:abc'})});
