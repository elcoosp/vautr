...
async function registerLogin(u, pw) {
  ...
  const sr = await req('POST', '/auth/register/start', {
    username: u,
    registration_start: b64(start.message),
  });
  const upload = w.opaque_register_finish_js(
    start.state,
...
async function main() {
  ...
  const se = await req(
    'POST',
    '/secrets',
    { project_uuid: proj.uuid, key: 'DB_PASS', value_ciphertext: b64('supersecret') },
    token,
  );
  ...
  const ma = await req(
    'POST',
    '/machine-accounts',
    { name: 'no-reveal', scopes: ['secrets:read'] },
    token,
  );
  ...
  const tok = await req(
    'POST',
    '/tokens',
    { name: 'nr-tok', machine_account_uuid: ma.body.uuid, scopes: ['secrets:read'] },
    token,
  );
