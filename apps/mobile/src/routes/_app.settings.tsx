...
function SettingsScreen() {
  ...
  const [createdToken, setCreatedToken] = useState<{ token: string; token_id: string } | null>(null);
  const [busy, setBusy] = useState(false);

  const load = useCallback(async () => {
    try {
      const [m, t] = await Promise.all([services.api.listMachineAccounts(), services.api.listTokens()]);
      setMachines(m);
  ...
  const createMachine = async () => {
    ...
    try {
      ...
      toast.show({ title: 'Failed to create machine account', description: err instanceof Error ? err.message : undefined, variant: 'destructive' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const createToken = async () => {
    setBusy(true);
    try {
      const result = await services.api.createToken({ name: 'mobile-access', scopes: ['secrets:read', 'secrets:reveal'] });
      setCreatedToken({ token: result.token, token_id: result.token_id });
      await load();
    } catch (err) {
      toast.show({ title: 'Failed to create token', description: err instanceof Error ? err.message : undefined, variant: 'destructive' });
    } finally {
      setBusy(false);
    }
  };
    ...
    <View className="gap-4">
      ...
      <Card className="p-4 gap-3">
        ...
        <View className="gap-1.5">
          ...
          <Input id="ma-name" value={name} onChangeText={setName} placeholder="ci-deploy" autoCapitalize="none" />
        </View>
        <View className="gap-1.5">
      ...
      <Card className="p-4 gap-3">
        ...
        {createdToken ? (
          <View className="gap-1">
            <Text className="text-xs text-muted-foreground">Copy your token now — it won't be shown again.</Text>
